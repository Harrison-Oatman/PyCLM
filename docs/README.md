...
| [testing-guide-stage6.md](testing-guide-stage6.md) | What to test for Stage 6: the camera ROI and skipping dry, the pixel calibrator, a 2 × 2 grid and a skip at the microscope. | Grids, skipping or the ROI change. |
| [release-wet-tests.md](release-wet-tests.md) | The checks that need a microscope before a release: environment, one known experiment end to end, closed loop, runtime changes, Stage 6, robustness, reading the results; what to record. | A feature that touches hardware is added (and add an item to `tests/wet_tests/`). |
| [stage6-z-grid-skip-design.md](stage6-z-grid-skip-design.md) | Stage 6 proposal: grid acquisition from MicroManager's tile-creator position list (one stitched frame per channel, one pattern cut per tile), the camera ROI, camera-space pattern storage with measured sizes, skipping idle positions at the SLM handshake, pattern review removed; z-stacks considered and dropped. Implemented 2026-09-11 (patterns kept in both DMD and camera space); implementation notes at the top. | The plan's event model, storage layout or the skip rule change. |
