...
| Microscope | `core/microscope.py` | `position` event → `PositionMover.move_to` (PFS variant locks focus and leaves it locked). `acquire` → config, exposure, binning, wait until scheduled, `waitForSystem`, settle, one `snapImage`. `update_pattern` → SLM handshake, `setSLMImage`. Acks only acquisitions (`EventDoneMessage`). |
