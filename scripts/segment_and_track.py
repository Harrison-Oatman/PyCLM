...
def parse_args():
    ...
    parser.add_argument(
        "glob",
        type="str",
        help="glob pattern to match files, e.g. '*_545.tif'",
        default="*.tif",
    )
    parser.add_argument(
        "--cellpose_norm",
        type=float,
        nargs="+",
        help="optionally specify low and high intensity values for cellpose: e.g. --cellpose_norm 0 5000",
        default=[],
    )
    parser.add_argument(
        "--model", type=str, help="cellpose model to use", default="cpsam"
    )
